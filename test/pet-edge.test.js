...
test('chooses the nearest work-area edge for an idle pet', () => {
  ...
  assert.equal(nearestDesktopEdge({ x: 800, y: 6, width: 160, height: 220 }, workArea), 'left');
});
...
test('edge head mode only exposes horizontal edges', () => {
  assert.equal(nearestDesktopEdge({ x: 850, y: 0, width: 120, height: 120 }, workArea), 'left');
});
