...
function edgeHeadBounds(edge, bounds, workArea, size) {
  ...
  const y = clamp(Math.round(centerY - headSize / 2), workArea.y, maxY);
  ...
  if (edge === 'top') return { x, y: workArea.y, width: headSize, height: headSize };
  return { x, y: maxY, width: headSize, height: headSize };
}
...
function restoredPetBounds(edge, headBounds, petBounds, workArea) {
  ...
  if (edge === 'top') return { x: clamp(centeredX, workArea.x, maxX), y: workArea.y, width: petBounds.width, height: petBounds.height };
  return { x: clamp(centeredX, workArea.x, maxX), y: maxY, width: petBounds.width, height: petBounds.height };
}
