...
const LEGACY_DEFAULT_DIFY_BASE_URL = 'http://dify.forcome.com/v1';
const DEFAULT_DIFY_BASE_URL = 'https://dify.forcome.com/v1';
...
const DEFAULT_RETRY_DELAYS_MS = [2 * 1000, 8 * 1000];
...
function normalizePolicySettings(value = {}) {
  ...
  const configuredBaseUrl = typeof source.baseUrl === 'string' && source.baseUrl.trim() ? source.baseUrl.trim().slice(0, 500) : '';
  const usesLegacyDefault = configuredBaseUrl === LEGACY_DEFAULT_DIFY_BASE_URL;
  return {
...
function requestDify(url, { body, apiKey, timeoutMs = DEFAULT_TIMEOUT_MS, maxResponseBytes = DEFAULT_MAX_RESPONSE_BYTES, signal } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => { request.destroy(); finish(policyError('aborted')); };
...
function isRetryableDifyError(error) {
  ...
  return code === 'timeout' || code === 'network-error' || /^http-(408|425|429|5\d\d)$/.test(code);
}

function waitForRetry(delayMs, signal) {
  return new Promise((resolve, reject) => {
    if (signal?.aborted) { reject(policyError('aborted')); return; }
    const timer = setTimeout(done, Math.max(0, Number(delayMs) || 0));
    const onAbort = () => { clearTimeout(timer); done(policyError('aborted')); };
    function done(error) {
...
async function syncDifyPolicy({ baseUrl, allowInsecureHttp = false, apiKey, signal, request = requestDify, timeoutMs = DEFAULT_TIMEOUT_MS, maxResponseBytes = DEFAULT_MAX_RESPONSE_BYTES, retryDelaysMs = DEFAULT_RETRY_DELAYS_MS } = {}) {
  let parsed;
  ...
  const retryDelays = Array.isArray(retryDelaysMs) ? retryDelaysMs : DEFAULT_RETRY_DELAYS_MS;
  ...
  for (let attempt = 0; attempt <= retryDelays.length; attempt += 1) {
    try {
      ...
      return { policy: parseDifyAnswer(payload.answer), rawBytes: Buffer.byteLength(response.body), attempts: attempt + 1, elapsedMs: Date.now() - startedAt };
    } catch (error) {
      if (signal?.aborted || !isRetryableDifyError(error) || attempt === retryDelays.length) {
