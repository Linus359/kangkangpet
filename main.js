...
const EMPLOYEE_POLICY_SYNC_TIMEOUT_MS = 60 * 1000;
const EMPLOYEE_POLICY_BACKGROUND_RETRY_DELAYS_MS = [2 * 60 * 1000, 10 * 60 * 1000, 30 * 60 * 1000, 2 * 60 * 60 * 1000];
...
function isRetryableEmployeePolicyError(errorCode) {
  return errorCode === 'timeout' || errorCode === 'network-error' || /^http-(408|425|429|5\d\d)$/.test(String(errorCode || ''));
}

function scheduleEmployeePolicySync({ retry = false } = {}) {
  clearTimeout(employeePolicyTimer);
  if (!config?.employeePolicy?.enabled) return;
  const delayMs = retry && employeePolicyRetryIndex < EMPLOYEE_POLICY_BACKGROUND_RETRY_DELAYS_MS.length
    ? EMPLOYEE_POLICY_BACKGROUND_RETRY_DELAYS_MS[employeePolicyRetryIndex++]
    : 24 * 60 * 60 * 1000;
  employeePolicyTimer = setTimeout(() => syncEmployeePolicy().catch(() => {}), delayMs);
  return delayMs;
}
...
async function syncEmployeePolicy() {
  ...
  employeePolicySyncPromise = (async () => {
    ...
    try {
      ...
      scheduleEmployeePolicySync();
      scheduleHandbookSprinkle();
      writeLog(`员工守则同步成功：版本 ${policy.policyVersion}，${result.attempts || 1} 次请求，耗时 ${result.elapsedMs || 0}ms。`);
      return { ok: true, source: 'dify' };
    } catch (error) {
      config.employeePolicy.errorCode = error?.code || 'network-error';
      saveEmployeePolicyStatus();
      const retryDelayMs = isRetryableEmployeePolicyError(config.employeePolicy.errorCode)
        ? scheduleEmployeePolicySync({ retry: true })
        : scheduleEmployeePolicySync();
      const retryMessage = retryDelayMs && retryDelayMs < 24 * 60 * 60 * 1000 ? `，将在 ${Math.round(retryDelayMs / 60000)} 分钟后重试` : '';
      writeLog(`员工守则同步失败：${config.employeePolicy.errorCode}，${error?.attempts || 1} 次请求，耗时 ${error?.elapsedMs || 0}ms${retryMessage}。`);
